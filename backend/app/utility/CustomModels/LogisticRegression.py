...
class LogisticRegression:
    ...
    # --------------------------
    def _sigmoid(self, z):
        # Clip z to prevent overflow and ensure numerical stability
        z = np.clip(z, -500, 500)
        # Use more stable sigmoid implementation
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_iters):
            ...
            if np.any(np.isnan(self.weights)) or np.isnan(self.bias):
                print("Warning: NaN detected in parameters, reinitializing...")
                self.weights = np.random.normal(0, 0.01, n_features)
                self.bias = 0.0
                break
    ...
    def get_parameters(self):
        ...
        if self.sklearn_model is not None and self.x_scaler is not None:
            ...
            return {
                "weights": _safe_float_list(weights_original),
                "bias": _safe_float(bias_original),
                "learning_rate": _safe_float(self.lr),
        ...
        if self.weights is None:
            ...
            weights_out = _safe_float_list(self.weights)
        else:  # scalar
            weights_out = [_safe_float(self.weights)]

        bias_out = _safe_float(self.bias) if self.bias is not None else 0.0
